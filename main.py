...
def start_game():
    canvas.fill(BLACK)
    start_font1 = large_font.render("Добро пожать в игру Змейка", True, BLUE)
    start_font2 = medium_font.render("Играть", True, BLACK, GREEN)
    start_font3 = medium_font.render("Инструкции", True, BLACK, GREEN)
    start_font4 = medium_font.render("Выход", True, RED, GREEN)
...
score_file.write("Ваш имя:" + str(user_name) + '\n')
score_file.close()

def save_score(score, line):
    with open('score.txt', 'a') as score_file:
        score_file.write("Ваш счет:" + str(score) + ' + ' + line + '\n')


def start_inst(start_font1, start_font1_rect):
    canvas.fill(BLACK)
    canvas.blit(start_font1, start_font1_rect)
    start_inst1 = small_font.render("--> Не заходите за стены", True, BLUE)
    start_inst2 = small_font.render("--> Съедайте колбочки с элементами", True, BLUE)
    start_inst3 = small_font.render("--> Не ходите сквозь себя", True, BLUE)
    start_inst4 = small_font.render("--> Играйте с удовольствием", True, BLUE)
...
def gameover():
    ...
    font_gameover1 = large_font.render('Конец игры', True, GREEN)
    font_gameover2 = medium_font.render("Играть снова", True, RED, YELLOW)
    font_gameover3 = medium_font.render("Выход", True, RED, YELLOW)
    ...
    while True:
        ...
        while True:
            ...
            if snake_head_rect.colliderect(apple_rect):
                ...
                APPLE_Y = random.randrange(TOP_WIDTH, WINDOW_HEIGHT - 10, 10)
                snakelength += 1
                line = str(random.choice(sentences))
                score = small_font.render("Score:" + str(snakelength - 3) + " " + line, True, YELLOW)
                final_score = str(snakelength - 3)
                save_score(final_score, line)
